...
p <- ggplot(comp_df, aes(x=site_id, y=total_biomass_gm2))
p <- p + geom_point(aes(shape=climate_source))
p <- p + facet_wrap(~year)
print(p)
pngname <- "C:/Users/Ginger/Dropbox/NatCap_backup/Mongolia/model_results/soum_centers/figs/August_biomass_2016_2017.png"
...
p <- ggplot(sub_res, aes(x=date, y=CHIRPS_minus_NAMEM))
p <- p + geom_line()
p <- p + facet_grid(variable~site_id, scales="free")
print(p)
pngname <- paste(figdir, "CHIRPS_minus_NAMEM.png", sep="/")
png(file=pngname, units="in", res=300, width=15, height=10)
print(p)
dev.off()
